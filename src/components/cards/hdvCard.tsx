...
export default async function HdvCard({ theme }: { theme: string }) {
    ...
    return (
        ...
        className={`${theme == "dark" ? "white_card" : "white_card_light grain" } col-span-1 relative p-2 rounded-3xl flex justify-center aspect-square`}>
                ...
                <Image placeholder="blur" blurDataURL="data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw=="src={"/hdv.webp"} width={500} height={500} className="absolute top-0 left-0 object-center object-cover -z-[1000] rounded-3xl opacity-60" alt="" loading="lazy" unoptimized={true}/>
        </a>
    );
}